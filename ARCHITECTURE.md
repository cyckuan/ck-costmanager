...
|-----------|-------------------|------|
| `/cost` command dispatch | Reads `commands/cost.md`, runs `node cost.js $ARGUMENTS` | LLM must correctly pass user arguments to bash |
...
Claude Code's plugin system requires a `.md` file for slash commands — there is no "pure code" command registration. The `.md` content is a prompt that the LLM interprets when the user invokes `/cost`. This is an unavoidable platform constraint.
...
commands/cost.md       # Slash command prompt (only LLM touchpoint)
```
